...
/*   Updated: 2018/06/05 21:16:55 by hypark           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int				ft_valid_check(char *str, int **col_arr, t_map *map)
{
	...
	if (*str == '\0' || *str == '\n')
	{
		ft_putstr("There should be at least one box!\n");
		return (0);
	}
	if (ft_char_check(str, map))
	{
		ft_putstr("There is unvalid character in the file!\n");
		return (0);
	}
	ft_row_newline(str, &row, &newline);
	if (row != newline)
	{
		ft_putstr("There is no line break in each of line\n");
		return (0);
	}
	if (row != map->line)
	{
		ft_putstr("Map line is different from its information!\n");
		return (0);
	}
	ft_col_arr(str, col_arr, row);
	if (ft_check_col(*col_arr, row))
	{
		ft_putstr("The column number is not same!\n");
		return (0);
	}
	return (row);
}
