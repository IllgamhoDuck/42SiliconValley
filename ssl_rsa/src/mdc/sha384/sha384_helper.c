...
/*   Updated: 2019/09/03 23:42:45 by hypark           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_sha384			*init_sha384(t_ssl *ssl)
{
	...
	if (ssl->p_mutual)
		sha384->len = ssl->mut_len;
	else
		sha384->len = ssl->read_len;
	sha384->str = (uint8_t *)ft_strnew(sha384->len);
	ft_memcpy(sha384->str, ssl->ssl_input, sha384->len);
	i = -1;
	while (++i < 8)
