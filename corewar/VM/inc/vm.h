...
/*   Updated: 2019/11/13 16:17:12 by anjansse         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
# define OP_T(x)	g_op_tab[x]

# define AC			cw->parsing.ac
# define AV			cw->parsing.av

# define OCP		(OP_T(cp->op).n_param > 1 && OP_T(cp->op).cbyte == 1)
# define TRUNC		(OP_T(cp->op).trunc == 0 && OP_T(cp->op).param[0] == T_DIR)
