...
/*   Updated: 2019/09/03 23:41:10 by hypark           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_sha224			*init_sha224(t_ssl *ssl)
{
	...
	if (ssl->p_mutual)
		sha224->len = ssl->mut_len;
	else
		sha224->len = ssl->read_len;
	sha224->str = (uint8_t *)ft_strnew(sha224->len);
	ft_memcpy(sha224->str, ssl->ssl_input, sha224->len);
	i = -1;
	while (++i < 8)
